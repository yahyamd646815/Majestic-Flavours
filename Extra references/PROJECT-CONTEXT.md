...
4. **In-app sign-up page** for new users, plus removing/migrating `sampleUsers.ts` to rely directly on Clerk. Yahya has explicitly confirmed he wants this even knowing it may introduce issues — he prefers it that way.
5. **Collapsible views** — the Admin/Manager "All Time" reports view grouped into month (probably week too) dropdowns; also collapse the units and categories sections on Settings. Yahya may add more Settings requests from his dad at this point.
6. **`expo-notifications`** for real push on stock-status pings — he wants this *before* the Users page work.
7. **Settings changelog / "what's new"** (explicitly "for later, not now") — a small or detailed note in Settings telling employees and Admin/Manager what changed in a recent update. Yahya's own idea: reuse the `--message` text he already writes for every `eas update`/`eas build` rather than requiring separate release notes. **Not yet verified:** whether that message text is actually readable by the app at runtime via `expo-updates`' own API, or whether this needs a separate mechanism (e.g. a small Supabase table Yahya writes to) — needs checking when this is actually scoped, not assumed.
8. **Lower priority, order flexible:**
